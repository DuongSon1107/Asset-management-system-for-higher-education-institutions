...
class Device_model extends CI_Model
{
    ...
    public function total_search($arSearch)
    {
        ...
        if (count($arSearch) > 0) {
            if ($arSearch['id_cat'] != "" && $arSearch['is_active'] != "" && $arSearch['name'] != "") {
                $arWhere = ['sotaisan.id_cat' => $arSearch['id_cat'], 'sotaisan.is_active' => $arSearch['is_active']];
                $arLike = ['sotaisan.name' => $arSearch['name']];
                $this->db->where($arWhere);
                $this->db->like($arLike);
                ...
                $arWhere = ['sotaisan.is_active' => $arSearch['is_active']];
                $this->db->where($arWhere);
            } else if ($arSearch['id_cat'] != "") {
                $arWhere = ['sotaisan.id_cat' => $arSearch['id_cat']];
                $this->db->where($arWhere);
    ...
    public function detail($id_sotaisan)
    {
        $this->db->select('*');
        $this->db->from("sotaisan");
        $arWhere = ['id_sotaisan' => $id_sotaisan, "is_active" => 1];
        $this->db->where($arWhere);
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    public function tinlienquan($id_sotaisan, $id_cat)
    {
        $this->db->select('*');
        $this->db->from("sotaisan");
        $arWhere = ['id_sotaisan!=' => $id_sotaisan, 'id_cat' => $id_cat, "is_active" => 1];
        $this->db->where($arWhere);
        $this->db->limit(3);
        $this->db->order_by('id_sotaisan', 'desc');
        $query = $this->db->get();
