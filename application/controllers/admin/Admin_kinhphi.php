...
class Admin_kinhphi extends MY_Controller
{
    ...
    public function update()
    {
        if ($this->input->post('update')) {
            ...
            if ($result) {
                $arKT = ["msg" => "Bổ sung kinh phí thành công. Vui lòng chờ quản trị xét duyệt", "kt" => 1];
                $this->session->set_userdata("kt", $arKT);
                redirect('/nguon-kinh-phi');
            } else {
                $arKT = ["msg" => "Có lỗi khi bổ sung ", "kt" => 2];
                $this->session->set_userdata("kt", $arKT);
                redirect('/nguon-kinh-phi');
            }
        }
    ...
    public function update_nguonkinhphi()
    {
        if ($this->input->post('update')) {
            ...
            if ($result) {
                $arKT = ["msg" => "Bổ sung kinh phí thành công.", "kt" => 1];
                $this->session->set_userdata("kt", $arKT);
                redirect('/nguon-kinh-phi');
            } else {
                $arKT = ["msg" => "Có lỗi khi bổ sung ", "kt" => 2];
