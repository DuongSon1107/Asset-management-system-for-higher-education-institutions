...
class Admin_login extends CI_Controller
{
            ...
            if (count($check_user) > 0) {
                ...
                if (count($result) > 0) {
                    ...
                    if (count($check) > 0) {
                        ...
                        $arKT = ["msg" => "Tài khoản của bạn chưa được kích hoạt! Vui lòng đăng nhập vào lúc khác"];
                        $this->session->set_userdata("kt", $arKT);
                        redirect("../dang-nhap.html");
                    }
                } else {
                    $arKT = ["msg" => "Bạn nhập sai tên đăng nhập hoặc pass"];
                    $this->session->set_userdata("kt", $arKT);
                    redirect("../dang-nhap.html");
                }
            } else {
                $arKT = ["msg" => "Bạn chưa có tài khoản! Vui lòng đăng ký"];
                $this->session->set_userdata("kt", $arKT);
                redirect("../dang-nhap.html");
            }
        }
    ...
    public function register()
    {
        ...
        if (count($arDK) > 0) {
            ...
            $arKT = ["msg" => "Đăng ký thành công. Vui lòng chờ được kích hoạt tài khoản"];
            $this->session->set_userdata("kt", $arKT);
            redirect("../dang-nhap.html");
        } else {
            $arKT = ["msg" => "Đăng ký thất bại"];
            $this->session->set_userdata("kt", $arKT);
            redirect("../dang-nhap.html");
        }
    }
